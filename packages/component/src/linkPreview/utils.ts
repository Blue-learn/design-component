...
export const getHtmlEntitiesDecodedText = (
	text?: string,
) => {
...
export const getPreviewData = async (
	...
	try {
		...
		const link =
			textWithoutEmails.match(REGEX_LINK)?.[0];
		...
		if (
			REGEX_IMAGE_CONTENT_TYPE.test(contentType)
		) {
		...
		while (
			(matches = REGEX_META.exec(head)) !== null
		) {
			meta.push([...matches]);
		}
		...
		const metaPreviewData = meta.reduce<{
			...
			(acc, curr) => {
				...
				const ogImage =
					!acc.imageUrl &&
					getContent(curr[2], curr[3], 'og:image');
				const ogTitle =
					!acc.title &&
					getContent(curr[2], curr[3], 'og:title');
				...
				return {
					description: description
						...
						: acc.description,
					imageUrl: ogImage
						? getActualImageUrl(url, ogImage)
						: acc.imageUrl,
					title: ogTitle
						? getHtmlEntitiesDecodedText(ogTitle)
						: acc.title,
				};
			},
		...
		if (!previewData.image) {
			...
			while (
				(imageMatches =
					REGEX_IMAGE_TAG.exec(html)) !== null
			) {
			...
			for (const tag of tags
				...
				.slice(0, 5)) {
				const image = await getPreviewDataImage(
					getActualImageUrl(url, tag[1]),
				);
